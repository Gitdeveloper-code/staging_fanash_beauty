...
const Page = () => {
 ...
 const BOOKINGS_API_URL = `${process.env.NEXT_PUBLIC_HOST_URL}/api/bookings`;
...
//   console.error("Error during booking:", errorMessage);
  ...
  useEffect(() => {
    const fetchServices = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_HOST_URL}/api/services`, {
          cache: "no-store",
        });
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
    ...
    const fetchBookingApi = async () => {
      try {
        const response = await fetch(`/api/availabity`, {
          cache: "no-store",
        });
        if (!response.ok) {
          throw new Error(`HTTP error! Status: ${response.status}`);
